...
6. Installable, product-neutral DSH bundles under `plugins/`.
...
Development always uses the freshly prepared vendored checkout, even when older release resources remain under `target/debug`. Every platform Release build deploys the CLI production closure, materializes workspace peer packages, stages the Desktop-only bridge under `desktop-plugins/`, all installable bundles under `plugins/`, plus the native Node executable, and stores the expanded Harness tree as a Tauri resource. Windows publishes a portable ZIP, Linux publishes AppImage and DEB bundles, and macOS publishes architecture-specific DMGs. The packaged process executes `runtime/harness/lib/bin.js` directly beside the embedded Node runtime, so startup does not unpack or copy a second runtime tree into the application's local data directory.

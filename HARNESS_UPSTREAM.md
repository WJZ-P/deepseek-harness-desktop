...
Only the Tauri integration bridge remains under [`desktop-plugins/`](desktop-plugins/): it synchronizes themes and adapts the Desktop launcher's file-URL plugin mounts to the same `dsh.client` boot-graph contract native DSH obtains from package resolution. Attachment storage, drag/drop UI, input previews, and history rendering ship as the standard [`dsh-attachments`](plugins/dsh-attachments/) bundle; the Input Modalities control ships as the standard [`dsh-model-capabilities`](plugins/dsh-model-capabilities/) bundle. Both public plugins live under `plugins/`, follow the native DSH bundle format, and can be installed without Desktop. None of those product implementations live in vendored Harness business packages.

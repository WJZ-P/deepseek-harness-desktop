...
**内置配置界面不编辑 `input`。** 它和 `compat`、`reasoningEfforts`、`thinkingBudgets`、`headers` 一样是 settings 文档字段，而模型列表编辑器仍是一张只覆盖 id、名称和两个容量的手写表单。编辑器声明了[模型字段扩展决策](2026-08-16-model-settings-field-extension-slot.md)所描述的可叠加子 slot：`settings.models.model.fields`，因此外部插件可以通过页面现有的行 patch 编辑 `input`，而无需让内置表单承担这项策略。那张卡片会承载自己并不编辑的字段：它的行 patch 会先展开已存储的行再应用改动，而采纳候选时已有行优先于重新发现的候选，因此手写或插件贡献的 `input` 在两条路径上都能存活。
...
自定义提供方下的视觉模型只需声明一次 `input: [text, image]`，可以通过模型字段插件写入，也可以直接写进 settings 文档——或者当该路由列出的模型全都接受图片时，在路由上写一行。这就是修复的全部：三个准入点随即接纳它上面的图片，`read_image` 也可用。什么都不写的部署保持原有行为完全不变，因此没有任何既有路由改变它报出的内容。

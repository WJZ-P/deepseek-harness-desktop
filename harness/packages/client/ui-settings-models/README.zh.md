...
每个展开的模型行都会渲染 `settings.models.model.fields` 列表型子 slot。贡献方会收到完整保留的模型草稿和仅作用于该行的 patch 回调，因此无需替换本页面或接入其 settings 写入流程，就能编辑适配器支持的可选字段。
...
- **内置卡片只编辑 API 密钥与精选折叠区字段**：手写编辑器用 schema 通用的字段覆盖面换来了设计稿上的布局（[Agent Note](../../../.agents/notes/implemented/architecture/2026-07-30-web-config-plane.md)）。两个家族都公开 `baseURL` 与模型的 `id`/`name`/`contextWindow`/`maxTokens`；手工声明的 pi-ai 路由还公开 `displayName` 与 `api`。模型行插件可通过 `settings.models.model.fields` 添加控件；重试策略、超时、DeepSeek 模型说明及其他进阶 profile 字段仍留在 `settings.yaml` 中。编辑器未展示的现有模型字段会予以保留。不带这些约定字段的 profile schema 只渲染该提示，两套精选布局则以 `llm-deepseek`/`llm-pi-ai` 这两个 namespace 的名字为键。
